...
# Note: this script used to pin the Node version firebase-tools ran under, because
# firebase-admin@11's dep chain reached SlowBuffer (removed in Node 22+) and the codebase
# analysis step died on newer runtimes. firebase-admin@14 no longer does, so the pin is
# gone -- deploy works on whatever Node your shell (or Homebrew's firebase-cli) provides.
